...
class Test(BaseTest):
    ...
    def test_dev_tool_export_dashboard_by_id(self):
        ...
        command = f"go run {command} -dashboard Metricbeat-system-overview -folder {folder_name}"
    ...
    def test_dev_tool_export_dashboard_by_id_from_space(self):
        ...
        command = f"go run {command} -dashboard Metricbeat-system-overview -space-id foo-bar -folder {folder_name}"
    ...
    def create_kibana_space(self):
        url = (
            f"http://{self.get_kibana_host()}:{self.get_kibana_port()}"
            + "/api/spaces/space"
        )
