...
class Test(BaseTest):
    ...
    def test_encodings(self):
        ...
        for _, enc_py, text in encodings:
            with codecs.open(self.working_dir + f"/log/test-{enc_py}", "w", enc_py) as f:
                f.write(text + "\n")

        # create the config file
        inputs = [
            {"path": self.working_dir + f"/log/test-{enc_py}", "encoding": enc_go}
        ...
        for _, enc_py, text in encodings:
            with codecs.open(self.working_dir + f"/log/test-{enc_py}", "a", enc_py) as f:
                f.write(f"{text} 2" + "\n")
