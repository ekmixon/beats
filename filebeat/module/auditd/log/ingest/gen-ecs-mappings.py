...
def extract_object(name: str, source: dict) -> dict:
    ...
    for k, v in source.items():
        if k in ['primary', 'secondary']:
            r[f'{name}.{k}'] = v
        elif k not in ['what', 'path_index', 'how']:
            raise Exception(f'Unexpected object key: {k}')
    return r
...
def map_object(instance: dict, context: str, mappings: dict):
    for k, v in instance.items():
        if k not in mappings:
            raise Exception(f'Unexpected key "{k}" while parsing {context}')
        mappings[k](k, v)
...
def convert_mappings(m: dict) -> dict:
    ...
    def store_ecs(k: str, v: dict):
        def store_mappings(k: str, v: list):
            if not isinstance(v, list):
                raise Exception(f'ecs.mappings must be a list, not {repr(v)}')
            nonlocal mappings
            mappings = v
...
if __name__ == '__main__':
    if len(sys.argv) != 2:
        print(f'Usage: {sys.argv[0]} <path/to/go-libaudit-repo>')
        sys.exit(1)
    repo_path = sys.argv[1]
    if not os.path.isdir(repo_path):
        raise Exception(f'Path to go-libaudit is not a directory: {repo_path}')
    git_path = f"{repo_path}/.git"
    if not os.path.isdir(git_path):
        raise Exception('go-libaudit directory doesn\'t contain a git repository: ' + git_path)
    norms_path = f"{repo_path}/aucoalesce/normalizations.yaml"
    ...
    revision = (
        check_output(
            f'git --work-tree={quote(repo_path)} --git-dir={quote(git_path)} describe --tags',
            shell=True,
        )
        .decode('utf8')
        .strip()
...
print(f'# AUTOGENERATED FROM go-libaudit {revision}, DO NOT EDIT')
yaml.safe_dump({
    'params': {
        'types': dict(types),
